...
public:
	float alpha;
	float radius;
	float alpha_minus_radius;
	GaussianFilter(float _alpha = 2.f, float _radius = 2.f)
	{
		alpha = _alpha;
		radius = _radius;
		alpha_minus_radius = expf(-alpha * radius * radius);
	}

	float G(float d) const { // from the slides, e^alpha * d^2 - e^-alpha * r^2
		return expf(-alpha * d * d) - alpha_minus_radius;
	}

	float filter(float x, float y) const
	{
		return G(x) * G(y); // gx * gy
	}
...
public:
	float B;
	float C;
	MitchellNetravali(float _B = 1.f/3.f, float _C = 1.f/3.f) {
		B = _B;
		C = _C;
	}

	float hm(float x) const { // copy paste formula from slides basically
		x = fabsf(x);
		if (x < 1.f) {
			return 1.f/6.f * ((12.f - 9.f*B - 6.f*C) * x * x * x + (-18.f + 12.f * B + 6.f * C) * x * x + (6.f - 2.f*B));
		}

		if (x < 2.f) {
			return 1.f/6.f * ((-B - 6.f*C) * x * x * x + (6.f * B + 30.f * C) * x * x + (-12.f * B - 48.f * C) * x + (8.f * B + 24.f * C));
		}
...
public:
	...
	void tonemap(int x, int y, unsigned char& r, unsigned char& g, unsigned char& b, float exposure = 1.0f)
	{
		...
		float w = filmic(11.2);
		float e = 1.f / 2.2f; // apply filmic formula to the film basically
